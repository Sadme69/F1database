...
# In-memory LRU cache for loaded sessions (with lock to prevent concurrent
# duplicate loads). Each loaded session holds ~0.8-1GB of telemetry in pandas,
# so this MUST be bounded — an unbounded cache OOMs after a few sessions.
# SESSION_CACHE_SIZE controls how many stay resident (default 1, suitable for a
# ~2GB box). The lock also serializes loads so two ~1GB loads never overlap.
from collections import OrderedDict

SESSION_CACHE_SIZE = max(1, int(os.environ.get("SESSION_CACHE_SIZE", "1")))
_session_cache: "OrderedDict[str, fastf1.core.Session]" = OrderedDict()
_session_lock = threading.Lock()


def _cache_get_session(key: str):
    """Return a cached session and mark it most-recently-used, else None."""
...
def _cache_put_session(key: str, session) -> None:
    """Insert a session and evict least-recently-used beyond the cap."""
    _session_cache[key] = session
    _session_cache.move_to_end(key)
    while len(_session_cache) > SESSION_CACHE_SIZE:
        old_key, _ = _session_cache.popitem(last=False)
        logger.info(f"Evicted session {old_key} from memory (LRU, cap={SESSION_CACHE_SIZE})")
...
def _load_session(year: int, round_num: int, session_type: str) -> fastf1.core.Session:
    ...
    sess = _cache_get_session(key)
    if sess is not None:
        return sess

    # The lock both prevents duplicate loads of the same session AND serializes
    # loads globally, so two ~1GB session loads never coexist in memory.
    with _session_lock:
        ...
        if len(session.laps) > 0:
            _cache_put_session(key, session)
...
def _telemetry_arrays(tel, driver: str, lap_number: int) -> dict | None:
    """Downsample one lap's telemetry frame to the compact, rounded chart arrays.

    Rounding to display precision removes float entropy so the stored file
    compresses ~6x instead of ~2.6x, with no visible change in the charts.
    """
    ...
    result = {
        "driver": driver,
        "lap": lap_number,
        "distance": t["Distance"].astype(float).round(1).tolist() if has_distance else list(range(len(t))),
        "speed": t["Speed"].astype(float).round(1).tolist(),
        "throttle": t["Throttle"].astype(float).round(1).tolist(),
        "brake": [int(b) * 100 for b in t["Brake"].tolist()],
        "gear": t["nGear"].astype(int).tolist(),
        "rpm": t["RPM"].astype(float).round().astype(int).tolist(),
    }
    if has_drs:
        result["drs"] = t["DRS"].astype(int).tolist()
    if "RelativeDistance" in t.columns:
        result["relative_distance"] = t["RelativeDistance"].astype(float).round(5).tolist()
    return result
...
def _get_driver_telemetry_sync(
    ...
    try:
        ...
        return None
    return _telemetry_arrays(tel, driver, lap_number)


def _get_driver_telemetry_all_laps_sync(
    year: int, round_num: int, session_type: str, driver: str
) -> dict[str, dict]:
    """Return telemetry for ALL of a driver's laps as {lap_number: arrays}.

    ~12x faster than calling the per-lap function in a loop: it merges the
    car + position channels ONCE for the whole driver, then slices each lap out
    of the merged frame (re-deriving per-lap distance), instead of re-merging on
    every lap.
    """
    session = _load_session(year, round_num, session_type)
    drv_laps = session.laps.pick_drivers(driver)
    if len(drv_laps) == 0:
        return {}
    try:
        full = drv_laps.get_telemetry()  # single merge for all of this driver's laps
    except Exception:
        return {}
    ...
    for _, lap in drv_laps.iterlaps():
        ...
        try:
            seg = full.slice_by_lap(lap)
        except Exception:
            continue
        if seg is None or len(seg) == 0:
            continue
        # Re-derive distance relative to this lap's start (the merged frame's
        # Distance is cumulative across the session).
        try:
            seg = seg.add_distance()
            seg = seg.add_relative_distance()
        except Exception:
            pass
        arrays = _telemetry_arrays(seg, driver, lap_number)
        if arrays:
            out[str(lap_number)] = arrays
    return out
