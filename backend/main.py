...
# Compress JSON/text API responses over the wire (telemetry, laps, etc.).
# WebSocket replay frames are unaffected (separate protocol).
app.add_middleware(GZipMiddleware, minimum_size=1024)

# CORS — needed when another site (your main website) calls this as an API
# cross-origin. Set ALLOWED_ORIGINS to a comma-separated list of origins, e.g.
# "https://example.com,https://www.example.com". Defaults to "*" (any origin).
# Same-origin use (frontend served by this backend) doesn't need any of this.
_origins_raw = os.environ.get("ALLOWED_ORIGINS", "*").strip()
_allow_origins = ["*"] if _origins_raw in ("", "*") else [o.strip() for o in _origins_raw.split(",") if o.strip()]
if _allow_origins == ["*"]:
